#include <stdint.h>
char* print_uuid_str(uint8_t *uuid, uint8_t len)
{
    struct uuid_str_t {
        int num;
        char *str;
    ...
    int i;
    for(i=0;i<sizeof(uuid_str)/sizeof(uuid_str[0]);i++){
        if(len == 2 && uuid[0] == (uuid_str[i].num & 0xFF) && uuid[1] == (uuid_str[i].num >> 8)){
            printf("(%s)", uuid_str[i].str);
