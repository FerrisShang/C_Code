...
static uint16_t p_hdl1;
static uint16_t p_hdl2;
    ...
    for(i=1;i<MAX_ATT_NUM;i++){
        switch(db[i].att_type){
            case AT_SERV:
                ...
                printf("  :0x%04X~0x%04X\n", db[i].handle, db[i].serv_end_hdl);
                break;
            case AT_CHARD:
                printf("C | 0x%04X ", db[i].handle);
                print_uuid(db[i].uuid, db[i].uuid_len);
                printf("  ");
                if(db[i].perm_read){ printf("R!"); }else{ printf("  "); }
                if(db[i].perm_write){ printf("W!  "); }else{ printf("    "); }
...
void db_read(uint8_t st_hdl, uint16_t offset)
{
    if(eb_gap_get_encrypted(conn_handle)){
        for(;st_hdl<=p_hdl2;st_hdl++){
            if(db[st_hdl].att_type == AT_CHARV && (db[st_hdl].perm_read)){break;}
        }
    }else{
        for(;st_hdl<=p_hdl2;st_hdl++){
    ...
    switch (param->evt_id) {
        ...
        case EB_EVT_GATTC_READ_GROUP_RSP: {
            ...
            for(int i=0;i<param->gattc.read_group.serv_num;i++){
                eb_gattc_service_t* p = &param->gattc.read_group.serv[i];
        ...
        case EB_EVT_GATTC_READ_BY_TYPE_RSP: {
            ...
            for(int i=0;i<param->gattc.read_by_type.char_num;i++){
                eb_gattc_character_t* p = &param->gattc.read_by_type.chars[i];
        ...
        case EB_EVT_GATTC_FIND_INFO_RSP: {
            ...
            for(p_hdl1+=1;p_hdl1<=p_hdl2;p_hdl1++){
                if(db[p_hdl1].att_type == AT_NONE){break;}
            }
            eb_gattc_find_info(conn_handle, p_hdl1, p_hdl1);
            break;
        }
            ...
            if (p->err_code == 0x0A) { // ATT not found
                switch (p->req_opcode) {
                    ...
                    case 0x04: { // Find Infomation request
                        if(p_hdl1 > p_hdl2){ // SDP done
                            db_read(1, 0);
                            return;
                        }else{
                            for(;p_hdl1<=p_hdl2;p_hdl1++){
                                if(db[p_hdl1].att_type == AT_NONE){break;}
                            }
                            eb_gattc_find_info(conn_handle, p_hdl1, p_hdl1);
                        }
