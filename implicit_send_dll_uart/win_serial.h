...
public:
    ...
    int read(uint8_t* buf, int len)
    {
        if (hComm) {
            ...
            if (stat.cbInQue >= len) {
                ReadFile(hComm, buf, len, &read_len, NULL);
            }
            return read_len;
        }
    }
...
public:
    int start(const char* port, int baudrate)
    {
        ...
        if (!read_thread) {
            read_thread = new thread(__read__, this);
        }
    ...
    void setCallback(void (*cb)(vector<uint8_t> &data, void* param), void* param)
    {
        callback = cb;
        cb_param = param;
    }
    ...
    {
        write((uint8_t*)&data[0], data.size());
    }
#define ERR_NO_ERR 0
#define ERR_FMT   -1
#define ERR_TOUT  -2
    ...
    static void __read__(CBtIO* btio)
    {
        ...
        while (1) {
            ...
            if (res > ERR_NO_ERR) {
                buf.resize(res);
                if (btio->callback) {
                    btio->callback(buf, btio->cb_param);
                } else {
